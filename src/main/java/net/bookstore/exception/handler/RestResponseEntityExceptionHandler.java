package net.bookstore.exception.handler;
...
import net.bookstore.common.api.ApiResponseHelpers;
...
import net.bookstore.exception.common.InvalidRequestDataException;
...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(@NonNull HttpMessageNotReadableException ex,
        ...
        if (ex.getCause() == null || ex.getCause() instanceof HttpMessageNotReadableException) {
            if (ex.getMessage() != null && ex.getMessage().matches(".*request.*body.*missing.*")) {
                body.put(StringConstants.CAUSE, "Request body is missing");
            }
        }

        // TYPE MISMATCH case
        else if (ex.getCause() instanceof MismatchedInputException mie) {
            body.put(StringConstants.CAUSE, defaultCause + ": mismatched input");
            ...
            body.put(StringConstants.FIELDS, Map.of(fieldName, expectedType + " is expected"));

        }

        // SYNTAX ERROR case
        else if (ex.getCause() instanceof JsonParseException) {
            body.put(StringConstants.CAUSE, defaultCause + ": syntax error");
        }
    ...
    protected @NonNull ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        body.put(StringConstants.FIELDS, fields);
    ...
    public ResponseEntity<Object> handleDataIntegrityViolation(DataIntegrityViolationException div) {
        ...
        if (div.getCause() instanceof ConstraintViolationException cve) {
            ...
            if (constraintName.startsWith(AbstractPersistentObject.PFX_UNIQUE)) {
                String cause = "Unique constraint violation";
                body.put(StringConstants.CAUSE, cause);
            ...
            // FOREIGN KEY constraint case
            else if (constraintName.startsWith(AbstractPersistentObject.PFX_FOREIGN)) {
                ...
                body.put(StringConstants.MSG, message);
                body.put(StringConstants.FIELDS, fieldNames);

                log.error(cause + ": " + fieldNames[0] + " -> " + fieldNames[1]);
            }

            // CHECK constraint case
            else if (constraintName.startsWith(AbstractPersistentObject.PFX_CHECK)) {
            ...
            // Unknown constraint violation
                ...
                body.put(StringConstants.CAUSE, cause);
                body.put(StringConstants.MSG, "Unknown data error");
            ...
            if (sqlException.getSQLState().equals("22003")) {
                body.put(StringConstants.CAUSE, "Numeric value out of range");
                body.put(StringConstants.MSG, localeUtils.getLocalizedMessage("error.value.numeric-value-out-of-range"));
            } else {
                body.put(StringConstants.MSG, "Unknown data exception");
            }
    ...
    protected @NonNull ResponseEntity<Object> handleMissingServletRequestParameter(@NonNull MissingServletRequestParameterException ex,
        ...
        body.put(StringConstants.FIELDS, Map.of(ex.getParameterName(), localeUtils.getLocalizedMessage("validation.empty")));
    ...
    public ResponseEntity<Object> handleInvalidRequestDataException(InvalidRequestDataException ex) {
        ...
        body.put(StringConstants.FIELDS, ex.getFieldInfo());
        return ResponseEntity.badRequest().body(body);
    }
    ...
    public ResponseEntity<Object> handleEntityNotFoundException(EntityNotFoundException ex) {
        ...
        body.put(StringConstants.MSG, localeUtils.getLocalizedMessage("error.entity-not-found"));
    ...
    private static String[] uniqueKeyToFieldNames(String ukName) {
        ...
        if (ukName.endsWith(AbstractPersistentObject.SFX_PKEY)) {
            keysNames = ukName.substring(0, ukName.length() - AbstractPersistentObject.SFX_PKEY.length()).split("_and_");
        } else {
            keysNames = ukName.substring(AbstractPersistentObject.PFX_UNIQUE.length()).split("__");
        }
        ...
        for (int i = 0; i < keysNames.length; i++) {
            ...
            if (key.endsWith(AbstractPersistentObject.SFX_ID)) {
                key = key.substring(0, key.length() - AbstractPersistentObject.SFX_ID.length());
            }
    ...
    private static String[] foreignKeyToEntities(String fkName) {
        ...
        String[] keyNames = fkName.substring(AbstractPersistentObject.PFX_FOREIGN.length()).split("__");
    ...
    private static String[] checkConstraintToFieldNames(String ckName) {
        String[] keyNames = ckName.substring(AbstractPersistentObject.PFX_CHECK.length()).split("__");
